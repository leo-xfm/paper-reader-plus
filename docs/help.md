...
- App version updated to `0.2.3`. / 应用版本更新为 `0.2.3`。
- Added a dedicated `Settings > PDF` panel for paragraph actions, author graph hovers, and internal PDF link / figure / table previews. / 新增 `Settings > PDF` 面板，可控制段落操作、作者图谱悬停，以及内部 PDF 链接 / 图 / 表预览。
- Added an Agent API max output token setting. Values greater than `0` are sent as `max_tokens` for chat and streaming requests. / Agent API 新增最大输出 token 设置；数值大于 `0` 时会作为 `max_tokens` 发送到普通聊天和流式请求。
- Live Markdown now supports foldable unordered list items, plus toolbar actions to expand or collapse all foldable lists. / Live Markdown 支持无序列表项折叠，并提供全部展开或全部折叠按钮。
- PDF, Markdown, table, and image toolbars now include "More tools" popovers for secondary actions. / PDF、Markdown、表格和图片工具栏新增“更多工具”弹出菜单，用于收纳次级操作。
- The library sidebar can be resized by dragging, and settings are edited as a draft until Save is clicked. / 左侧文库侧栏支持拖拽调整宽度；设置在点击保存前以草稿方式编辑。
- Markdown code ligature replacement now includes `+-` to `±`, with clearer settings text. / Markdown 代码连字替换新增 `+-` 到 `±`，设置说明也更明确。
- Store migration, tests, and English/Simplified Chinese localization were expanded for the new settings. / 已为新设置补充数据迁移、测试和中英文界面文案。
...
5. Open `Settings > General`, `Settings > PDF`, `Settings > Markdown`, `Settings > Agent API`, `Settings > OCR API`, and `Settings > Translation API` before deeper use.
...
5. 深入使用前，建议先配置 `Settings > General`、`Settings > PDF`、`Settings > Markdown`、`Settings > Agent API`、`Settings > OCR API` 和 `Settings > Translation API`。
...
- Optional PDF interaction toggles for paragraph actions, author graph hovers, and internal PDF link / figure / table previews. / 可选关闭段落操作、作者图谱悬停，以及内部 PDF 链接 / 图 / 表预览。
...
- Fold unordered list items in Live mode and expand or collapse all foldable lists from the toolbar. / 在 Live 模式折叠无序列表项，并可通过工具栏全部展开或全部折叠。
...
The AI panel and selection actions use an OpenAI-compatible Chat Completions API. The current UI preset provider is `volcengine`; the service layer currently accepts only `agent_api_type = chat`. `ai_max_output_tokens` controls the optional `max_tokens` request field; use `0` to omit it.

AI 面板和选区动作使用 OpenAI-compatible Chat Completions API。当前 UI 预设 provider 为 `volcengine`，服务层当前只接受 `agent_api_type = chat`。`ai_max_output_tokens` 用于控制可选的 `max_tokens` 请求字段；设为 `0` 表示不发送。
...
- `Settings > PDF`: paragraph actions, author graph hovers, and internal PDF link / figure / table previews. / 配置段落操作、作者图谱悬停，以及内部 PDF 链接 / 图 / 表预览。
- `Settings > Markdown`: default editor mode, Markdown font size, Markdown line height, Western body font family, Chinese body font family, code font family, code line height, code ligatures, ReaderM edit split default, preview position, code line numbers, highlight syntax/color, math rendering, live HTML rendering, and Live-mode list folding. / 配置默认编辑模式、Markdown 字号、行高、西文正文字体、中文正文字体、代码字体、代码行高、连字、ReaderM 分屏默认值、预览位置、代码行号、高亮语法/颜色、数学渲染、实时 HTML 渲染和 Live 模式列表折叠。
- `Settings > Agent API`: provider, base URL, model, max output tokens, API key, professional field, research area, summary source, figure attachment limit, and AI context options. / 配置 AI provider、Base URL、模型、最大输出 token、API key、专业领域、研究方向、摘要来源、图像附件数量和 AI 上下文选项。
...
| PDF settings / PDF 设置 | `Settings > PDF` | - |
